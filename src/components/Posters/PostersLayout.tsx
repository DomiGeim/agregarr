...
const PostersLayout = ({ children }: PostersLayoutProps) => {
  ...
  return (
    <>
      ...
      <div className="mt-6">
        <nav className="hide-scrollbar flex gap-2 overflow-x-auto rounded-lg bg-stone-900/40 p-1.5">
          {tabs.map((tab) => {
            const isActive = router.pathname.match(tab.regex);
            return (
              <Link key={tab.route} href={tab.route}>
                <a
                  className={`min-w-max rounded-md px-4 py-3 text-sm font-semibold leading-6 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-opacity-75 sm:px-6 sm:text-base ${
                    isActive
      ...
      <div className="mt-6 text-white sm:mt-8">{children}</div>
    </>
  );
};
